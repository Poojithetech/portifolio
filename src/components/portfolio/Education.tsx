      ...
      <div className="relative max-w-3xl mx-auto">
        ...
        <div className="space-y-10">
          {items.map((it, i) => {
            const isLeft = i % 2 === 0;
            const card = (
              <div className="glass rounded-2xl p-6 glow-hover w-full">
                ...
                <h3 className={`font-serif text-2xl mb-1 ${isLeft ? "md:text-right" : ""}`}>{it.title}</h3>
                <p className={`text-sm text-muted-foreground ${isLeft ? "md:text-right" : ""}`}>{it.place}</p>
                <div className={`mt-4 flex ${isLeft ? "md:justify-end" : ""}`}>
            ...
            return (
              <div key={it.title} className="reveal relative md:grid md:grid-cols-2 md:gap-12 items-center">
                {/* Dot */}
                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-gradient-to-br from-primary to-accent shadow-[0_0_20px_hsl(var(--primary)/0.7)] z-10" />
                {isLeft ? (
                  <>
                    <div className="pl-12 md:pl-0 md:pr-12">{card}</div>
                    <div className="hidden md:block" />
                  </>
                ) : (
                  <>
                    <div className="hidden md:block" />
                    <div className="pl-12 md:pl-12 md:pr-0">{card}</div>
                  </>
                )}
              </div>
            );
